...
ExporterLAZ::ExporterLAZ(std::string path)
{
    ...
    std::cout << "LASzip DLL version (maj.min.rev.build): " << int(version_major) << "." << int(
            version_minor) << "." << int(version_revision) << "." << int(version_build) << std::endl;
    ...
    // https://www.asprs.org/a/society/committees/lidar/Downloads/ASPRS_LAS%201_2.pdf
    // populate the header
    ...
    auto[year, month, day, yday] = getDate();
    header_->file_creation_day = yday;
