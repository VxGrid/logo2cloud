...
bool MainWindow::eventFilter(QObject *watched, QEvent *event)
{
   ...
   else if(event->type() == QEvent::GraphicsSceneMousePress)
   {
       ...
       {
           if (!setSelectedPixelColorTempToAlpha0(mouseEvent)) return false;
       }
   ...
   {
       QGraphicsSceneMouseEvent *mouseEvent = dynamic_cast<QGraphicsSceneMouseEvent*>(event);
       if (!translatesceneItemsByMouseDrag(mouseEvent)) return false;
...
bool MainWindow::loadFile(const QString &fileName)
{
    ...
    image_ = newImage;
    if (image_.colorSpace().isValid()) image_.convertToColorSpace(QColorSpace::SRgb);
...
void MainWindow::addImage2Scene(const QImage &newImage)
{
    ...
    QGraphicsPixmapItem *graphPixItem = new QGraphicsPixmapItem(QPixmap::fromImage(image_));
    graphPixItem->setZValue(1); // for identification and that the loaded image is on top, so alpha works
...
bool MainWindow::setSelectedPixelColorTempToAlpha0(QGraphicsSceneMouseEvent *mouseEvent)
{
    ...
    for(const auto &graphItem : sceneItems)
    {
        ...
        statusBar()->showMessage(tr("Clicked pixel [x|column ; y|row]: %1 ; %2").arg(point.x()).arg(point.y()));

        QGraphicsPixmapItem* graphPixItem = dynamic_cast<QGraphicsPixmapItem*>(graphItem);
        if(!graphPixItem) return false;
...
bool MainWindow::translatesceneItemsByMouseDrag(QGraphicsSceneMouseEvent *mouseEvent)
{
    QPointF trans = mouseEvent->scenePos()-mouseEvent->lastScenePos();

    if(trans.isNull()) return false;
