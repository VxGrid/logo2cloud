    ...
    image_{image}
{
    ...
    ui_->l_exportFormat->hide();
    ui_->cb_exportFormat->hide();
#endif
    ...
    connect(sh, &QShortcut::activated, [this]() {
        ui_->pb_removeColor->animateClick();
    });
...
void Editor::updateImage(QImage &image, QColor colBefore, QColor colAfter)
{
    ...
    for (int r = 0; r < image.height(); ++r)
    {
        for (int c = 0; c < image.width(); ++c)
        {
            ...
            if (imgBits[index] == blue && imgBits[index + 1] == green && imgBits[index + 2] == red
                && imgBits[index + 3] == alpha)
            {
...
void Editor::onPropertyChanged()
{
    ...
    else if (sender() == ui_->sb_height || sender() == ui_->sb_width || sender() == ui_->sb_depth
        || sender() == ui_->sb_pointSpacing)
    {
        ...
        (sender() == ui_->sb_height) ? ui_->sb_width->setValue(ratio * ui_->sb_height->value()) :
        ui_->sb_height->setValue(ui_->sb_width->value() / ratio);
        calculateSetPointSpacing();
        setPCloudProperties();
    ...
    else if (sender() == ui_->pb_export)
    {
        ...
        pcloudgen_.setFileFormat2Use(PointCloudGenerator::EXPORTER(ui_->cb_exportFormat->currentIndex()));
        QString fPath = "tempPointCloud";
        ...
        do
        {
            qDebug() << "Progress: " << pcloudgen_.getProgressPercent() << Qt::endl;
        ...
        QFile file(fPath + "." + ui_->cb_exportFormat->currentText());

        if (!file.open(QFile::ReadOnly)) qDebug() << "File open failed" << Qt::endl;
...
// Chose path
        ...
        for (const auto &fileFormats : pcloudgen_.fileFormats())
            filters.push_back(QString::fromStdString(fileFormats));
        ...
        if (dialog.exec() == QDialog::Accepted)
        {
            ...
            pcloudgen_.setData(img.bits(), img.height(), img.width());
            pcloudgen_.setRandomizeValue(ui_->sb_randomizer->value());
            pcloudgen_.setFileFormat2Use(dialog.selectedNameFilter().toStdString());
...
void Editor::updateMaxPointGeneratedSBox()
{
    ...
    double maxPoints = (ui_->sb_width->value() / pSpacing) * (ui_->sb_height->value() / pSpacing) *
        std::max(1, int(std::ceil(ui_->sb_depth->value() / pSpacing)));
